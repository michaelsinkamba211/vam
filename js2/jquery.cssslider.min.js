(function ($, window, document, undefined) {
  ...
  Plugin.prototype.setItem = function (n) {
    ...
    var wrapFunc;
    if (this.options.infiniteScroll) wrapFunc = this._wrapIndex;
    else wrapFunc = wrapFuncNone;
    for (var i = 0; i < sliderItems.length; i++) {
      var item = sliderItems.eq(i);
      if (i == n) item.addClass("current_item");
      else if (i < n) item.addClass("previous_hidden");
      else if (i > n) item.addClass("next_hidden");
    }
    if (this.options.infiniteScroll) {
      sliderItems
        .eq(this._wrapIndex(n - 1))
        .removeClass()
        .addClass("previous_item");
      sliderItems
        .eq(this._wrapIndex(n + 1))
        .removeClass()
        .addClass("next_item");
      if (this.options.visibleItems == 3) {
        sliderItems
          .eq(this._wrapIndex(n - 2))
          .removeClass()
          .addClass("previous_hidden");
        sliderItems
          .eq(this._wrapIndex(n + 2))
          .removeClass()
        ...
        sliderItems
          .eq(this._wrapIndex(n - 2))
          .removeClass()
          .addClass("previous_item_2");
        sliderItems
          .eq(this._wrapIndex(n + 2))
          .removeClass()
          .addClass("next_item_2");
        sliderItems
          .eq(this._wrapIndex(n - 3))
          .removeClass()
          .addClass("previous_hidden");
        sliderItems
          .eq(this._wrapIndex(n + 3))
  ...
  Plugin.prototype.nextItem = function () {
    if (this.inTransition) {
      if (this.commandQueue.length < 3) this.commandQueue.push("nextItem");
      return;
    }
    if (
      this.options.infiniteScroll ||
      this.currentItem < this.numSliderItems - 1
    ) {
      this.currentItem += 1;
  ...
  Plugin.prototype.prevItem = function () {
    if (this.inTransition) {
      if (this.commandQueue.length < 3) this.commandQueue.push("prevItem");
      return;
    }
  ...
  Plugin.prototype._wrapIndex = function (n) {
    if (n < 0) n += this.numSliderItems;
    if (n >= this.numSliderItems) n -= this.numSliderItems;
    return n;
  };
